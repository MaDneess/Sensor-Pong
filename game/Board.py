...
class Board:
                ...
                if event.type == QUIT:
                    ...
                    if event.key == K_ESCAPE:
                        if self.state != Commons.IN_MENU:
                            self.state = Commons.IN_MENU
                            self.closeCommPorts()
                    elif (event.key == K_RETURN) and ((self.state == Commons.IN_GAME_CAL) or (self.state == Commons.IN_MENU_CAL)):
                        if (not self.pBlue.device.isOpen()):
                            ...
                            StaticUtils.printMessage(CommUtils.CMD_OK, "Player BLUE calibrated. MID POINT: " + str(self.pBlue.device.DATA_MIDDLE_POINT))
    ...
    def drawBoard(self):
        ...
        pg.draw.line(self.display, Commons.BLACK, (int(Commons.B_WIDTH/2-2), 0), (int(Commons.B_WIDTH/2 - 2), Commons.B_HEIGHT), 4)
